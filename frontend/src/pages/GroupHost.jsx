        ...
        {!started ? (
          <>
            ...
            <Button data-testid="start-group-session" onClick={startSession} disabled={!session?.players?.length} className="w-full bg-gradient-to-r from-emerald-500 to-teal-600 text-white text-lg py-6">
              <Play className="w-5 h-5 mr-2" />{t('group.start_session')} ({session?.players?.length || 0} {t('group.players_label')})
            </Button>
          </>
        ) : (
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <Card className="p-4 bg-white/10 backdrop-blur-md border-white/20">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-white font-bold text-sm">{t('group.live_ranking')}</h3>
              </div>
              <div className="space-y-2">
                ...
                {sortedPlayers.length === 0 && <p className="text-blue-200 text-xs text-center">{t('group.waiting_answers')}</p>}
              </div>
            </Card>
          </div>
        )}
