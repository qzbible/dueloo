...
const Dashboard = () => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (dailyRes.data.can_play) {
        setTimeout(() => setShowDailyManna(true), 1000);
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #1E3A8A 0%, #312E81 50%, #1E3A8A 100%)' }}>
          ...
          <Card className="p-6 bg-white/10 backdrop-blur-md border-white/20">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div className="flex-1">
                <h2 data-testid="user-name" className="text-2xl font-bold text-white mb-1" style={{ fontFamily: 'Manrope, sans-serif' }}>
                  {user?.name}
                </h2>
                <div className="flex items-center gap-2">
                  <Trophy className="w-5 h-5 text-yellow-400" />
                  <span data-testid="user-level" className="text-lg text-yellow-300 font-semibold">Niveau {user?.level}</span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-3 gap-4 mt-4">
              <div className="text-center p-3 rounded-lg bg-white/5">
                ...
                <div className="text-xs text-blue-200">Vies</div>
              </div>
              ...
              <div className="text-center p-3 rounded-lg bg-white/5">
                ...
                <div className="text-xs text-blue-200">Pièces</div>
              </div>
              ...
              <div className="text-center p-3 rounded-lg bg-white/5">
                ...
                <div className="text-xs text-blue-200">Série</div>
              </div>
            </div>
          </Card>
        </motion.div>
            ...
            <Card className="p-6 bg-white/10 backdrop-blur-md border-white/20 h-full hover:bg-white/15 transition-all cursor-pointer" onClick={() => navigate('/campaign')}>
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <p className="text-blue-200">Traversez la Bible livre par livre</p>
                </div>
                <BookOpen className="w-12 h-12 text-yellow-400" />
              </div>
              <Button data-testid="play-campaign-button" className="w-full bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white">
                Jouer maintenant
              </Button>
            </Card>
          </motion.div>
            ...
            <Card className="p-6 bg-white/10 backdrop-blur-md border-white/20 h-full hover:bg-white/15 transition-all cursor-pointer" onClick={() => navigate('/premium')}>
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <p className="text-blue-200">Accès illimité et fonctionnalités exclusives</p>
                </div>
                <Crown className="w-12 h-12 text-yellow-400" />
              </div>
              <Button data-testid="premium-button" className="w-full bg-gradient-to-r from-yellow-400 to-yellow-600 hover:from-yellow-500 hover:to-yellow-700 text-gray-900 font-bold">
                Voir les offres
              </Button>
            </Card>
          </motion.div>
          ...
          <Card className="p-6 bg-white/10 backdrop-blur-md border-white/20">
            ...
            {badges.length > 0 ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-blue-200">Jouez pour débloquer vos premiers badges !</p>
              </div>
            )}
          </Card>
        </motion.div>
      ...
      {showDailyManna && (
        <DailyMannaModal 
          onClose={() => setShowDailyManna(false)} 
